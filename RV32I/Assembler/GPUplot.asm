...
li32 t5, "\0\0\00" ; numberが0の場合は、"0"を代入してstrASCIIは終了
sh t5, t4, 0
...
call setjump(plotString, plotDEC.plot + 3)
...
; a0 : when 0 is 画面内で終了
;    : when 1 is 画面外で強制終了
plotDECu: ; int plotDEC(int a5, int a4, int a7)[]
li t6, 0  ; t6 : カウントアップ変数
li t5, 0  ; t5 : あまり、ASCIIコード格納変数
li t4, 10 ; t4 : 一時定数、文字列ポインタ
.strnumloop:
...
addi sp, sp, -4
sw ra, sp, 0 ; plotDEC関数のraを退避
call setjump(plotString, plotDECu.plot + 3)
lw ra, sp, 0 ; plotDEC関数のraを復帰
addi sp, sp, 4 ; spを復帰
